const { entityName, site } = require("./config");
...
function getDailyAvailabilityCountsInCalendar(root) {
    ...
    try {
        const timeSelection = root.querySelectorAll(".time-selection");

        dailySlotCountsMap = timeSelection
            .map((timeSelection) =>
                timeSelection.getAttribute("data-readable-date")
            )
            .reduce((acc, date) => {
                const reformattedDate = moment(`${date}T00:00:00`).format(
                    "M/D/YYYY"
                );
                acc.set(reformattedDate, (acc.get(reformattedDate) || 0) + 1);
                return acc;
            }, new Map());
    } catch (error) {
